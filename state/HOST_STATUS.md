...
| `WS2_WINDOWS` | Windows host layer / not reported | not reported | No committed verification evidence yet. Treat Windows and Ubuntu/WSL2 as separate environments. | Pull `main`, read this file, audit the Windows host layer without recording private addresses or identifiers, and report the result in this row. | not reported |
