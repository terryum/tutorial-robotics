...
Enlight 접촉 제어기를 평가하고 후보 설정을 기록합니다.
...
실제 탐침·테이블 힘, 유한하고 제한된 제어, 렌더링 형상을 요구합니다. candidate-controller.json과 contact-actions.npz를 읽고 hardware_compatible은 false로 둡니다.
...
공개 Enlight 동역학과 로컬 탐침·테이블 장면에서 상태·접촉력을 얻습니다. Torch가 CUDA에서 PD·편향 보상·자코비안 힘을 계산합니다. 알려진 이득의 제어기 후보이며 학습 정책 checkpoint는 아닙니다.

$$
\tau=80(q^*-q)-12\dot q+c+g+J^TF
$$

기호·단위·조건: q rad; qdot rad/s; Kp Nm/rad; Kd Nm s/rad; F 세계 N.

손으로 계산하는 예시(실행 측정값이 아님): 80 × 0.01 − 12 × 0.02 = 0.56 Nm before bias and contact terms.

실전 연결: 알려진 이득으로 만든 제어기 candidate이며 학습 정책은 아닙니다.

## Code connection

`examples/sim-enlight-02/run.py` → `src/pai_lab/lessons/runner.py` → `src/pai_lab/lessons/gpu.py`. runner는 실행을 기록하고 실험 모듈이 실제 상태에서 수치를 계산합니다. `experiment.json`의 payload를 계산 코드와 대조합니다.
...
```bash
pal lesson review sim-enlight-02 --run-dir .local/runs/sim-enlight-02/baseline-01 --comparison-run-dir .local/runs/sim-enlight-02/comparison-01 --notes "측정 결과와 한 변수 비교를 설명하고 그래프와 렌더링을 확인했습니다." --json
