...
장비 setup·매뉴얼 참조: [Wuji Hand 2 Beta 2](../setup/wuji.md), [Sharpa Wave](../setup/sharpa.md).
...
기호·단위·조건: p: 정렬된 좌표계 m; q: rad; λ: m²/rad²; 관절 범위 내 탐색.

손으로 계산하는 예시(실행 측정값이 아님): position residual 0.01 m → squared term 0.0001 m².

실전 연결: 정확히 retarget해도 형태 차이로 잔여 오차가 남습니다.
