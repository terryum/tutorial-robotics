...
실제 ROS 메시지를 기록하고 rosbag2 데이터베이스를 재생합니다.
...
messages_written과 messages_replayed를 비교하고 episode_bag을 확인합니다. 실제 저장 메시지가 없는 재생은 통과할 수 없습니다.
...
```bash
pal lesson inspect sim-ros-03 --run-dir .local/runs/sim-ros-03/baseline-01 --json
```

Mac 결과 그래프 열기(창을 닫아도 실행 기록은 유지됩니다):

```bash
open .local/runs/sim-ros-03/baseline-01/plot.png
```

다른 OS의 파일 관리자에서는 같은 PNG를 엽니다. 원시 수치는 아래 JSON에 있습니다.

```bash
python -m json.tool .local/runs/sim-ros-03/baseline-01/experiment.json
```

## How it works

writer가 CDR 직렬화 JointState를 rosbag2 SQLite에 저장합니다. reader는 저장 바이트를 역직렬화해 상태 순서를 검사합니다. 예전 operation 이름에는 MCAP가 있지만 이 기준 실행은 실제 storage_id를 sqlite3로 기록합니다.
...
`examples/sim-ros-03/run.py` → `src/pai_lab/lessons/runner.py` → `src/pai_lab/lessons/ros.py`. runner는 실행을 기록하고 실험 모듈이 실제 상태에서 수치를 계산합니다. `experiment.json`의 payload를 계산 코드와 대조합니다.
...
```bash
pal lesson compare sim-ros-03 --run-dir .local/runs/sim-ros-03/baseline-01 --comparison-run-dir .local/runs/sim-ros-03/comparison-01 --output-dir .local/comparisons/sim-ros-03/comparison-01 --json
```
...
```bash
pal lesson review sim-ros-03 --run-dir .local/runs/sim-ros-03/baseline-01 --comparison-run-dir .local/runs/sim-ros-03/comparison-01 --notes "측정 결과와 한 변수 비교를 설명하고 그래프와 렌더링을 확인했습니다." --json
