...
장비 setup·매뉴얼 참조: [ALOHA](../setup/aloha.md).
...
기호·단위·조건: H: 시간 단계; da: 14 구동기 목표; 단위는 행동 계약 참조.

손으로 계산하는 예시(실행 측정값이 아님): 16 × 14 = 224 action values per chunk.

실전 연결: schema와 카메라 프레임만으로 ACT 학습을 완료한 것은 아닙니다.
