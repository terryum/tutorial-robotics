...
이 수업은 **Enlight 읽기 전용 어댑터와 저위험 검증**을 `reader-hardware-gate` 구현과 `no run artifact until the gate is implemented` artifact로 검사합니다.
...
먼저 `pal lesson check hw-enlight-01 --json`을 실행합니다. capability가 없으면 `pal setup verify --profile hardware --json`의 missing 목록을 따르고, 시스템 패키지나 firmware는 자동 설치하지 않습니다.
...
`pal lesson run hw-enlight-01 --headless --seed 8 --samples 96`로 seed와 표본 수를 바꿉니다. 새 artifact의 digest와 `changed_metric_value`가 달라지되 schema는 같아야 합니다.
...
과정의 다음 catalog 항목: [hw-enlight-02](./hw-enlight-02.md) — Enlight system identification and MuJoCo calibration
