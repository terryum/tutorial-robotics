...
고정된 공개 Wuji URDF를 실제 Isaac articulation으로 가져옵니다.
...
일치하는 관절 이름 20개를 요구하며 imported.usda와 isaac-frame.png를 확인합니다. 개수 일치는 첫 검사일 뿐이므로 방향과 크기도 눈으로 확인합니다.
...
URDF import가 관절·관성·형상을 USD/PhysX로 변환합니다. 버전 adapter는 5.1 Kit API 또는 6.0 직접 URDFImporter API를 사용합니다. mesh 경로는 로컬 파생 URDF에서 해석하고 vendor 파일은 보존합니다. 버전 호환성은 WS1 실행 증거로 확인해야 합니다.

$$
J_{matched}=J_{source}\cap J_{imported}
$$

기호·단위·조건: J: 여기서는 Jacobian이 아닌 이름 집합; 축·단위·질량·제한은 별도 비교.

손으로 계산하는 예시(실행 측정값이 아님): 20 expected joint names, 19 matched → one missing joint: fail.

실전 연결: USD 저장 성공만으로 import 충실도를 검증할 수 없습니다.

## Code connection

`examples/sim-isaac-01/run.py` → `src/pai_lab/lessons/runner.py` → `src/pai_lab/lessons/isaac.py`. runner는 실행을 기록하고 실험 모듈이 실제 상태에서 수치를 계산합니다. `experiment.json`의 payload를 계산 코드와 대조합니다.
...
```bash
pal lesson review sim-isaac-01 --run-dir .local/runs/sim-isaac-01/baseline-01 --comparison-run-dir .local/runs/sim-isaac-01/comparison-01 --notes "측정 결과와 한 변수 비교를 설명하고 그래프와 렌더링을 확인했습니다." --json
