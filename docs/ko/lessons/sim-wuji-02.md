...
학습한 Wuji checkpoint를 재로딩하고 측정 평가를 묶습니다.

## Preflight

장비 setup·매뉴얼 참조: [Wuji Hand 2 Beta 2](../setup/wuji.md).
...
평균 보상, evaluation-actions.npz, evaluation.png, candidate/manifest.json을 읽습니다. 실제 checkpoint 재로딩과 유한한 평가를 요구하며 performance_verified는 false로 둡니다.
...
학습과 같은 vendor 등록 과제에서 후보를 평가합니다. 해시가 평가와 정확한 정책 바이트를 연결합니다. embodiment 식별과 실물 보정은 별도 검사이므로 후보가 자동으로 Beta 2 실물 호환이 되지 않습니다.
...
`examples/sim-wuji-02/run.py` → `src/pai_lab/lessons/runner.py` → `src/pai_lab/lessons/gpu.py`. runner는 실행을 기록하고 실험 모듈이 실제 상태에서 수치를 계산합니다. `experiment.json`의 payload를 계산 코드와 대조합니다.
...
```bash
pal lesson review sim-wuji-02 --run-dir .local/runs/sim-wuji-02/baseline-01 --comparison-run-dir .local/runs/sim-wuji-02/comparison-01 --notes "측정 결과와 한 변수 비교를 설명하고 그래프와 렌더링을 확인했습니다." --json
