...
You will build a reproducible minimum baseline for **FR3 read-only state, shadow, and approved low-risk motion** and inspect the flow from reference state to observed state through numeric artifacts.
...
Run `pal lesson check hw-fr3-01 --json` first. If a capability is unavailable, follow the missing list from `pal setup verify --stage hardware --json`; do not auto-install system packages or firmware.
...
Run `pal lesson run hw-fr3-01 --headless --seed 8 --samples 96`. The digest and `changed_metric_value` should change while the artifact schema stays fixed.
...
Next catalog item: [hw-wuji-01](./hw-wuji-01.md) — Wuji Beta 2 read-only adapter and tactile calibration
