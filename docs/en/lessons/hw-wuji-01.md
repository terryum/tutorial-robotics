...
You will build a reproducible minimum baseline for **Wuji Beta 2 read-only adapter and tactile calibration** and inspect the flow from reference state to observed state through numeric artifacts.
...
Run `pal lesson check hw-wuji-01 --json` first. If a capability is unavailable, follow the missing list from `pal setup verify --stage hardware --json`; do not auto-install system packages or firmware.
...
Run `pal lesson run hw-wuji-01 --headless --seed 8 --samples 96`. The digest and `changed_metric_value` should change while the artifact schema stays fixed.
...
Next catalog item: [hw-wuji-02](./hw-wuji-02.md) — Wuji shadow and gated real evaluation
