...
Review the observations, prerequisites and per-run safety boundary for wuji beta 2 read-only adapter and tactile calibration.
...
pal lesson run hw-wuji-01 --headless --json
```
...
`examples/hw-wuji-01/run.py` → `src/pai_lab/lessons/runner.py`;
