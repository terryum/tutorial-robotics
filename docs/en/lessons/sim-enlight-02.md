...
You will build a reproducible minimum baseline for **Enlight contact-task simulation and candidate bundle** and inspect the flow from reference state to observed state through numeric artifacts.
...
Run `pal lesson check sim-enlight-02 --json` first. If a capability is unavailable, follow the missing list from `pal setup verify --stage sim --json`; do not auto-install system packages or firmware.
