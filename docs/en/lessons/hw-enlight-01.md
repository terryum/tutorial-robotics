...
You will build a reproducible minimum baseline for **Enlight read-only adapter and low-risk validation** and inspect the flow from reference state to observed state through numeric artifacts.
...
Run `pal lesson check hw-enlight-01 --json` first. If a capability is unavailable, follow the missing list from `pal setup verify --stage hardware --json`; do not auto-install system packages or firmware.
