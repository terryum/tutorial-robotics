...
You will build a reproducible minimum baseline for **ROS 2 topics, QoS, services, actions, and TF** and inspect the flow from reference state to observed state through numeric artifacts.
...
Run `pal lesson check sim-ros-01 --json` first. If a capability is unavailable, follow the missing list from `pal setup verify --stage sim --json`; do not auto-install system packages or firmware.
