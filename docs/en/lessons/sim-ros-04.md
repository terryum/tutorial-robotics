...
You will build a reproducible minimum baseline for **Common embodiment API for simulation and ROS 2** and inspect the flow from reference state to observed state through numeric artifacts.
...
Run `pal lesson check sim-ros-04 --json` first. If a capability is unavailable, follow the missing list from `pal setup verify --stage sim --json`; do not auto-install system packages or firmware.
