...
You will build a reproducible minimum baseline for **FR3 kinematics, Jacobian, and inverse kinematics** and inspect the flow from reference state to observed state through numeric artifacts.
...
Run `pal lesson check core-fr3-04 --json` first. If a capability is unavailable, follow the missing list from `pal setup verify --stage core --json`; do not auto-install system packages or firmware.
