...
You will build a reproducible minimum baseline for **Enlight system identification and MuJoCo calibration** and inspect the flow from reference state to observed state through numeric artifacts.
...
Run `pal lesson check hw-enlight-02 --json` first. If a capability is unavailable, follow the missing list from `pal setup verify --stage hardware --json`; do not auto-install system packages or firmware.
...
Run `pal lesson run hw-enlight-02 --headless --seed 8 --samples 96`. The digest and `changed_metric_value` should change while the artifact schema stays fixed.
...
Next catalog item: [hw-enlight-03](./hw-enlight-03.md) — Enlight contact-task shadow and gated evaluation
